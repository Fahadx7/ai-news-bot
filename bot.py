...
• تحليل يضيف قيمة فوق الخبر الخام — هذا ما يميزك
...
{{
  ...
  "headline": "عنوان صحفي قوي 50-85 حرف - يبدأ بالفاعل أو الحدث",
  "hook": "جملة افتتاحية تلخص الخبر في سطر 100-180 حرف",
  "details": "4-6 جمل 450-650 حرف - أرقام وأسماء دقيقة وسياق مختصر - أسلوب صحفي واضح",
  "why_matters": "ليش الخبر يهم المطور/المستثمر العربي - جملتان (180-280 حرف)",
  "reading_between_lines": "تحليل استراتيجي عميق - النوايا، المنافسة، الاتجاه (180-280 حرف)",
  "ready_tweet": "تغريدة عربية جاهزة 200-260 حرف - بدون هاشتاقات داخلها - أسلوب إنساني محترف",
  "hashtags": ["7 هاشتاقات مختارة للنمو على Twitter"],
  "related_video_id": <vid أو null>,
  "importance": 1-10
}}

استراتيجية الهاشتاقات (للنمو الحقيقي على Twitter):
1. هاشتاقان كبيران (وصول ضخم): #AI + #ChatGPT أو #Tech
2. هاشتاقان نوعيان (مجتمع AI): #GenAI #LLM #AIAgents #MachineLearning #DeepLearning #GenerativeAI
3. هاشتاق للشركة: #OpenAI #Claude #Gemini #Llama (حسب الخبر)
4. هاشتاق عربي كبير: #الذكاء_الاصطناعي أو #تقنية
5. هاشتاق نوعي (حسب الفئة): #Funding #AIResearch #AIPolicy
6. إذا وجد في الترند هاشتاق تقني مناسب — أدرجه

قواعد التغريدة:
• تبدو مكتوبة من خبير عربي في AI وليست ترجمة آلية
• تبدأ بشيء يلفت الانتباه: رقم، سؤال، ملاحظة حادة
• تنتهي بفكرة قوية أو خلاصة، لا سؤال عام
...
def news_cycle() -> None:
    ...
    for item in final_items:
        try:
            ...
            if 0 <= nid < len(sampled):
                articles_for_images.append(sampled[nid])
        except (ValueError, TypeError):
            articles_for_images.append({})
    ...
    for idx, item in enumerate(final_items):
        try:
            ...
            # استراتيجية الصور:
            # 1. إذا فيه og:image → استخدمه مع link_preview
            # 2. وإلا إذا فيه فيديو YouTube → استخدم الفيديو
            # 3. وإلا → نص فقط
            og_image = images.get(idx)
            preview_url = None
            if og_image:
                preview_url = original["url"]  # Telegram يسحب og:image منه
            elif video_url:
