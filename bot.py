...
- {THREAD_COUNT} خبر مهم يستحق thread شرح عميق (thread)

قواعد الاختيار:
✅ تنويع المصادر — لا خبرين من نفس الشركة
✅ أولوية لأخبار ذات تأثير مباشر على المستخدم السعودي والعربي
✅ تنويع بين: منتجات جديدة، أبحاث، إعلانات، تحليلات

{news_text}

رد بـ JSON فقط بدون أي نص آخر:
[{{
  "title_ar": "عنوان قصير جذاب باللهجة السعودية (مثل: قولك يا ناس، شيء ما تتوقعه)",
  "body_ar": "شرح الخبر في 2-3 جمل بالعربي الواضح",
  "tweet_main": "تغريدة رئيسية باللهجة السعودية — جذابة — أقل من 220 حرف — بدون هاشتاقات",
  "source": "اسم المصدر",
...
الخبر الأخير يكون content_type=thread."""
...
def run():
    ...
    for idx, item in enumerate(selected):
        ...
        if is_thread:
            ...
            preview = "\n\n".join(f"{i+1}/{len(parts)} {p}" for i, p in enumerate(parts))
            tg_send(f"🧵 *Thread نُشر ({len(parts)} تغريدات):*\n\n`{preview[:700]}`")
        else:
            tweet = f"{item['emoji']} {item['tweet_main']}\n\n{hashtags}\n\n🔗 {item['link']}"
            post_tweet(tweet[:280])
            tg_send(f"📋 *نُشر على X:*\n\n`{tweet[:300]}`")
