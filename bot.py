...
def news_cycle() -> None:
    ...
    # Diversify sampling
    sampled = diversify_articles(unique, target=80)
    print(f"[news] بعد sampling متنوع: {len(sampled)} مقال")

    # YouTube + Trends
    videos = fetch_youtube_videos(hours=72)[:35]
    ...
    raw = call_groq(
        ...
        NEWS_USER.format(
            ...
            trends=json.dumps(world_trends, ensure_ascii=False),
        ),
        temperature=0.5,
        max_tokens=6500,
    )
...
def report_cycle() -> None:
    ...
    # diversify للتقرير أيضاً
    sampled = diversify_articles(articles, target=60)
    compact = [{
        "title": a["title"][:240],
