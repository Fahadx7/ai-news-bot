...
NEWS_SOURCES: Dict[str, str] = {
    ...
    # ════════════════════════════════════════
    # Tier 11: المصادر السعودية التقنية (جديد)
    # ════════════════════════════════════════
    "أرقام": "https://www.argaam.com/ar/rss/articles",
    "سبق الإلكترونية": "https://sabq.org/rss.xml",
    "Saudi Press Agency": "https://www.spa.gov.sa/rss.xml",
    "Asharq Al-Awsat Tech": "https://aawsat.com/feed",
    "عكاظ": "https://www.okaz.com.sa/rss",
    "الاقتصادية": "https://www.aleqt.com/rss.xml",
    "Saudipedia": "https://saudipedia.com/feed",
    "Arab Tech News": "https://www.arabtechnews.net/feed",
    "البلاد": "https://www.albiladpress.com/rss.xml",
    "Trade Arabia Tech": "https://www.tradearabia.com/rss/news_tech.xml",
    "Al Riyadh Tech": "https://www.alriyadh.com/rss.xml",
    "Argaam Business": "https://www.argaam.com/ar/rss/companies",
    "Saudi Startups": "https://saudistartup.com/feed",
    "Gulf News Tech": "https://gulfnews.com/rss?section=technology",
    "Khaleej Times Tech": "https://www.khaleejtimes.com/rss/technology",
    "Entrepreneur AR": "https://www.entrepreneur.com/ar/topic/ذكاء-اصطناعي.rss",
    "مجلة لها التقنية": "https://www.lahamag.com/rss.xml",
    "StartupScene": "https://www.startupscene.me/rss",
    "Saudi Research News": "https://www.aliqtisadi.com/category/technology/feed",
    "Asharq Business": "https://asharq.com/ar/rss.xml",
    "Gulf Business Tech": "https://gulfbusiness.com/tag/technology/feed/",
    "Saudi Tech News": "https://saudinewsrss.com/technology",
}
...
GOOGLE_NEWS_QUERIES: List[Tuple[str, str, str]] = [
    ...
    # 🇸🇦 المحلي السعودي (جديد - الأولوية)
    ("الذكاء الاصطناعي السعودية", "ar", "SA"),
    ("هيوماين HUMAIN", "ar", "SA"),
    ("SDAIA سدايا", "ar", "SA"),
    ("رؤية 2030 تقنية", "ar", "SA"),
    ("نيوم تقنية ذكاء اصطناعي", "ar", "SA"),
    ("ستارت اب سعودي تقني", "ar", "SA"),
    ("صندوق الاستثمارات العامة تقنية", "ar", "SA"),
    ("LEAP 2026 السعودية", "ar", "SA"),
    ("Saudi AI HUMAIN", "en-US", "SA"),
    ("Saudi Arabia tech investment", "en-US", "SA"),
    ("Aramco digital technology", "en-US", "SA"),
    ("Neom Tonomus AI", "en-US", "SA"),

    # 🌍 خليجي
    ("تقنية الإمارات الذكاء الاصطناعي", "ar", "AE"),
    ("G42 UAE AI", "en-US", "AE"),
    ("تقنية قطر", "ar", "QA"),
...
def fetch_worldwide_trends() -> Dict[str, List[str]]:
    """جلب ترند من USA + السعودية منفصلين، cache 3 ساعات."""
    try:
        with open(TRENDS_CACHE_FILE, "r") as f:
            cached = json.load(f)
            if time.time() - cached.get("ts", 0) < 10800:
                return {
                    "world": cached.get("world", []),
                    "saudi": cached.get("saudi", []),
                }
    except (FileNotFoundError, json.JSONDecodeError):
        pass

    def scrape_trends(url: str) -> List[str]:
        try:
            r = requests.get(
                url,
    ...
    world = scrape_trends("https://trends24.in/united-states/")
    saudi = scrape_trends("https://trends24.in/saudi-arabia/")
    print(f"[trends] عالمي: {len(world)} · سعودي: {len(saudi)}")

    try:
        with open(TRENDS_CACHE_FILE, "w") as f:
            json.dump({"ts": time.time(), "world": world, "saudi": saudi}, f)
    except Exception:
        pass
    return {"world": world, "saudi": saudi}
...
def filter_relevant_trends(trends: List[str], max_n: int = 6) -> List[str]:
    ...
    tech_keywords = ["ai", "tech", "chatgpt", "gpt", "llm", "gemini",
                     ...
                     # سعودي
                     "سعودي", "السعودية", "تقنية", "ذكاء", "رؤية",
                     "نيوم", "هيوماين", "سدايا", "leap", "neom",
                     "humain", "saudi", "aramco"]
    relevant = []
    for tag in trends:
        lower = tag.lower()
...
def extract_youtube_id(url: str) -> Optional[str]:
    """استخراج ID فيديو YouTube من أي صيغة URL."""
    if not url:
        return None
    patterns = [
...
def get_youtube_thumbnail(url: str) -> Optional[str]:
    """بناء thumbnail URL من فيديو YouTube."""
...
def is_clean_url(url: str) -> bool:
    """تحقق: هل الرابط نظيف صالح لتويتر (ليس Google News redirect)."""
    if not url or not url.startswith("http"):
        return False
    if "news.google.com" in url:
        return False
    if len(url) > 400:  # رابط طويل جداً = redirect احتمالاً
        return False
    return True
...
{{
  ...
  "tweet_text": "نص تغريدة 200-240 حرف - بأسلوب سعودي حديث - يبدأ برقم/سؤال/ملاحظة - لا هاشتاقات ولا روابط داخل النص",
  "hashtags": ["7 هاشتاقات حقيقية للنمو على Twitter"],
  "related_video_id": <vid أو null>,
  "prefer_video_in_tweet": true/false,
  "importance": 1-10
}}

⚡ قاعدة prefer_video_in_tweet (مهم):
• true: إذا الخبر عن فيديو/عرض/demo/شرح مرئي → الأولوية للفيديو في التغريدة
• false: إذا الخبر نصي (تقرير، تمويل، سياسة، بحث) → الأولوية لرابط المقال الأصلي

🏷️ قواعد الهاشتاقات الصارمة:
• استخدم فقط هاشتاقات حقيقية وكاملة — ممنوع الهاشتاقات المقطوعة
• الهاشتاقات الإنجليزية (4-5 منها من القائمة الآمنة):
...
• الهاشتاقات العربية/السعودية (2-3 من القائمة الآمنة):
   #الذكاء_الاصطناعي  #ذكاء_اصطناعي  #تقنية  #تكنولوجيا
   #السعودية  #الرياض  #نيوم  #رؤية_2030  #هيوماين  #سدايا  #LEAP2026
   #اقتصاد_المعرفة  #ستارتب_سعودي  #تقنية_السعودية
...
• لا تضع روابط أو هاشتاقات داخل tweet_text — سيضافا تلقائياً
...
SAFE_ARABIC_HASHTAGS = {
    ...
    "#تحول_رقمي", "#رؤية_2030", "#تقنيات",
    # 🇸🇦 سعودية محلية (جديد)
    "#السعودية", "#الرياض", "#جدة", "#نيوم", "#نيوم_السعودية",
    "#هيوماين", "#سدايا", "#LEAP", "#LEAP2026",
    "#اقتصاد_المعرفة", "#الاقتصاد_الرقمي", "#المواهب_الرقمية",
    "#تقنية_السعودية", "#السعودية_2030", "#ستارتب_سعودي",
    "#أرامكو_الرقمية", "#تونومس", "#منصة_أبشر",
    # خليجية
    "#الإمارات_التقنية", "#دبي_للذكاء_الاصطناعي", "#G42",
}
...
ARABIC_HASHTAG_TOKENS = {
    ...
    "رؤية", "2030", "تقنيات", "2026",
    # سعودية
    "السعودية", "الرياض", "جدة", "الدمام", "نيوم",
    "هيوماين", "سدايا", "LEAP", "اقتصاد", "المعرفة",
    "الاقتصاد", "الرقمي", "المواهب", "أرامكو", "تونومس",
    "سعودي", "سعودية", "خليجي", "خليجية",
    "الإمارات", "دبي", "قطر", "الكويت", "البحرين",
}
...
def build_twitter_ready_tweet(
    ...
    """بناء نص تغريدة جاهز للصق في تويتر.

    تويتر يعمل unfurling (بطاقة إعلامية) من آخر رابط في التغريدة.
    القواعد:
    - إذا prefer_video=True وفي فيديو → رابط الفيديو في الأخير
    - وإلا → رابط المصدر في الأخير (تويتر يسحب og:image منه)
    - الهاشتاقات قبل الرابط
    - الرابط الأخير هو الذي يصبح بطاقة البريفيو

    🚨 مهم: الروابط لازم تكون نظيفة (مو Google News redirect).
    """
    # تنظيف الهاشتاقات
    clean_tags, seen = [], set()
    for tag in hashtags:
    ...
    # اختيار الرابط الرئيسي للـ unfurl
    # ⚠️ لازم يكون URL نظيف — ما يحتوي news.google.com
    primary_link = ""
    if prefer_video and video_url and "news.google.com" not in video_url:
        primary_link = video_url
    elif source_url and "news.google.com" not in source_url:
        primary_link = source_url
    elif video_url and "news.google.com" not in video_url:
        primary_link = video_url
    # إذا كل الروابط Google News redirects، لا نضع أي رابط
    # (أفضل من وضع رابط طويل ومعقد ما يعطي بطاقة)

    # بناء النص
    parts: List[str] = [tweet_text.strip()]
    if tag_line:
        parts.append(tag_line)
    if primary_link:
        parts.append(primary_link)
...
def format_news(
    ...
    """تنسيق الرسالة في Telegram — تغريدة جاهزة صالحة للصق مباشرة في تويتر."""
    ...
    tweet_text = (item.get("tweet_text", "") or item.get("ready_tweet", "") or "").strip()
    hashtags = item.get("hashtags", []) or []
    prefer_video = bool(item.get("prefer_video_in_tweet", False))
    ...
    # التغريدة الجاهزة — صالحة للصق مباشر في تويتر
    if tweet_text:
        full_tweet = build_twitter_ready_tweet(
        ...
        blocks += [sep, f"<code>{esc(full_tweet)}</code>"]
...
def telegram_send_smart(
    message: str,
    photo_url: Optional[str] = None,
) -> bool:
    """الدالة الذكية: إرسال Telegram بالطريقة المثلى.

    • إذا عندنا photo_url → sendPhoto مع caption (صورة + نص تحتها)
    • وإلا → sendMessage بدون أي معاينة (نص فقط)

    ⚠️ نتجنب link_preview لأنه يظهر تحت الرسالة = تكرار عنوان الخبر.
    """
    # إذا الرسالة طويلة، sendPhoto caption حدها 1024 بس
    # في هذه الحالة نقسم: صورة + نص قصير، ثم نص كامل في رسالة ثانية
    if photo_url:
        # طول caption الآمن
        caption_limit = 1020
        if len(message) <= caption_limit:
            return telegram_send_photo(photo_url, message)
        else:
            # الخبر طويل: نرسل الصورة + أول جزء كـ caption
            # ونُرسل الباقي كرسالة منفصلة بدون preview
            # لكن الأفضل: نرسل الصورة بدون caption ثم الرسالة كاملة
            # أو: نقسم الرسالة بذكاء عند separator
            sep_pos = message.find("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬")
            # نأخذ header + headline + first part of details كـ caption
            if sep_pos > 0:
                first_part_end = message.find("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬",
                                               sep_pos + 20)
                if 0 < first_part_end < caption_limit:
                    caption = message[:first_part_end].rstrip()
                    rest = message[first_part_end:].lstrip()
                    ok1 = telegram_send_photo(photo_url, caption)
                    if ok1 and rest:
                        time.sleep(1.5)
                        return telegram_send_message_plain(rest)
                    return ok1
            # fallback: رسالة كاملة بدون صورة
            return telegram_send_message_plain(message)
    else:
        return telegram_send_message_plain(message)

def telegram_send_message_plain(message: str) -> bool:
    """إرسال رسالة نصية بدون أي معاينة روابط."""
    if len(message) > 4050:
    ...
    try:
        ...
        if r.status_code == 200:
            return True
        print(f"[tg-msg] {r.status_code}: {r.text[:200]}")
        return False
    except Exception as exc:
        print(f"[tg-msg] {exc}")
        return False
...
def telegram_send_message(message: str, preview_url: Optional[str] = None,
    ...
    """إرسال رسالة مع صورة كبيرة من preview_url (إن وجد) — الطريقة القديمة."""
    if len(message) > 4050:
        message = message[:4040] + "…"
...
def news_cycle() -> None:
    ...
    trends_data = fetch_worldwide_trends()
    world_trends = filter_relevant_trends(trends_data["world"], max_n=4)
    saudi_trends = filter_relevant_trends(trends_data["saudi"], max_n=4)
    all_trends = saudi_trends + world_trends  # السعودي أولاً (أولوية)
    print(f"[trends] سعودي: {saudi_trends[:3]} · عالمي: {world_trends[:3]}")
    ...
    for idx, item in enumerate(final_items):
        try:
            ...
            # الرابط النظيف للمصدر (بعد حل redirect)
            clean_source = final_urls.get(idx, "") or original["url"]
            if not is_clean_url(clean_source):
                clean_source = ""  # لا نضعه في التغريدة إذا Google News

            # قرار الأولوية في التغريدة
            prefer_video = bool(item.get("prefer_video_in_tweet", False))

            # تمرير الروابط النظيفة فقط لـ format_news
            message = format_news(
                item,
                video_url=video_url,
            ...
            # ─────────────────────────────────────────
            # اختيار أفضل صورة (thumbnail مباشر، لا link preview)
            # ─────────────────────────────────────────
            photo_url = None

            # الأولوية 1: خبر فيديو → YouTube thumbnail
            if prefer_video and video_url:
                photo_url = get_youtube_thumbnail(video_url)

            # الأولوية 2: og:image من المقال (إذا الرابط نظيف)
            if not photo_url and is_clean_url(clean_source):
                photo_url = images.get(idx)

            # الأولوية 3: أي فيديو مرتبط (fallback)
            if not photo_url and video_url:
                photo_url = get_youtube_thumbnail(video_url)

            # إرسال ذكي: sendPhoto لو عندنا صورة، sendMessage لو لا
            ok = telegram_send_smart(message, photo_url=photo_url)
